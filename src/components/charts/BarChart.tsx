...
import { AreaChart as RechatsStackedAreaChart, BarChart as RechartsBarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Legend } from 'recharts';
...
const BarChart: React.FC<BarChartProps> = (props) => {
    ...
    function normalizeData(data: IStackedAreaChart[]): INormalizedData[] {
        ...
        validateData(data);
        // console.log(`[BarChart] normalizeData input: ${JSON.stringify(data?.slice(0, 2), null, 2)}`);
        ...
        // console.log(`[BarChart] normalizeData result: ${JSON.stringify(result?.slice(0, 2), null, 2)}`);
        return result;
    }
    ...
    const extractAttibutesNames = (normalizedData: INormalizedData[]): string[] => {
        ...
        if (normalizedData && normalizedData.length > 0) {
            // console.log('[BarChart] extractAttibutesNames:', JSON.stringify(normalizedData, null, 2));
            ...
            // console.log(`[BarChart] extractAttibutesNames Final attribute names: ${JSON.stringify(attrNames, null, 2)}`);
            result = attrNames;
        } else {
            console.warn('[BarChart] extractAttibutesNames: normalizedData is empty or undefined');
        }
        return result;
    };
    ...
    useEffect(() => {
        try {
            if (!props.data || props.data.length === 0) {
                console.warn('[BarChart]: data is required at first useEffect stage! It should be loaded from props.data.');
                setLoading(false);
                return;
            } else {
